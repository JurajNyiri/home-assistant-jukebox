class JukeboxCard extends HTMLElement {
  ...
  buildSpeakerSwitches(hass) {
    ...
    this._tabs.addEventListener("iron-activate", (e) =>
      this.onSpeakerSelect(e.detail.item.entityId)
    );
  ...
  buildStationList() {
    ...
    this.config.links.forEach((linkCfg) => {
      const stationButton = this.buildStationSwitch(
        linkCfg.name,
        linkCfg.url,
        linkCfg.logo
      );
  ...
  buildVolumeSlider() {
    ...
    this._hassObservers.push((hass) => {
      ...
      slider.value = speakerState.volume_level
        ? speakerState.volume_level * 100
        : 0;
  ...
  updateStationSwitchStates(hass) {
    ...
    if (
      hass.states[selectedSpeaker] &&
      hass.states[selectedSpeaker].state === "playing"
    ) {
    ...
    this._stationButtons.forEach((stationSwitch) => {
      if (
        stationSwitch.hasAttribute("raised") &&
        stationSwitch.stationUrl !== playingUrl
      ) {
    ...
    this.hass.callService("media_player", "play_media", {
      ...
      extra: {
        metadata: {
          ...
          artist: "Live Radio",
          images: [{ url: e.currentTarget.stationLogo }],
        },
      },
    });
  }
  ...
  findFirstPlayingIndex(hass) {
    return Math.max(
      0,
      this.config.entities.findIndex((entityId) => {
        return (
          hass.states[entityId] && hass.states[entityId].state === "playing"
        );
      })
    );
  }
